...
bool VolumetricFullVideoResource::read_file(std::ifstream &file){
    ...
    for(auto &cameraData : camData){
        ...
        for(size_t ii = 0; ii < static_cast<size_t>(nbFrames); ++ii){
            cameraData.frames.emplace_back(FrameData{0,0, std::make_shared<CompressedFullFrame>()});
        }
    ...
    for(auto &cameraData : camData){
        for(auto &frame : cameraData.frames){
            ...
            // read frame
            read_frame(file, reinterpret_cast<CompressedFullFrame*>(frame.data.get()));
        }
    }
...
void VolumetricFullVideoResource::write_file(std::ofstream &file){
    ...
    for(const auto &cameraData : camData){
        for(const auto &frame : cameraData.frames){
            ...
            // write frame
            write_frame(file, reinterpret_cast<CompressedFullFrame*>(frame.data.get()));
        }
    }
}

CompressedFullFrame *VolumetricFullVideoResource::get_full_frame(size_t idFrame, size_t idCamera){
    auto frame = get_frame(idFrame, idCamera);
    if(frame != nullptr){
        return reinterpret_cast<CompressedFullFrame*>(frame);
    }
    return nullptr;
}
